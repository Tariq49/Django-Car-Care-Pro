...
def customer_list_create(request):
    if request.method == 'GET':
        user = request.user
        print('Authenticated user ID:', user.id)  # Debug: check if the authenticated user ID matches
        customers = Customer.objects.filter(user=user)
        print('Found customers:', customers)  # Debug: verify if customers are fetched
